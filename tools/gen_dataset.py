...
def main(args):
    ...
    for TASK in TASKS:
        ...
        labels_dir = os.path.join(data_path, f"labels_yolo_MONO_2D/{TASK}")
        ...
        with Pool(NUM_THREADS) as pool:
            pbar = pool.imap(
                check_label_files, zip(img_paths, label_paths)
            )
        ...
        save_dir = os.path.join(data_path, "annotations")
        if not os.path.exists(save_dir):
            os.mkdir(save_dir)
        save_path = os.path.join(
            save_dir, "instances_" + f"{TASK}" + ".json"
        )
        generate_coco_format_labels(
