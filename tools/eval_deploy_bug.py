...
preds_coco = []
ann_id = 0
for i, pred_3d in enumerate(tqdm(preds_3d)):
    # if len(pred_3d):
    for pd in pred_3d:
        cls, _, _, _,  x1, y1, x2, y2, W, H, L, X, Y, Z, Ry, score = pd
...
gt_file = 'datasets/Rope3D/annotations_2.5D/instances_val.json'
coco_gt = COCO(gt_file)

_, tmp = tempfile.mkstemp()
json.dump(preds_coco, open(tmp, "w"))
# json.dump(preds_coco, open("./yolox_testdev.json", "w"))
# coco_pred = coco_gt.loadRes("datasets/Rope3D/predictions_2D/instances_val.json")
coco_pred = coco_gt.loadRes(tmp)
# 初始化评估器
